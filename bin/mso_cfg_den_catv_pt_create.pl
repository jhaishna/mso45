...
while ($line) 
{
    ...
    else 
    {
        ...
        ($PROV_TAG,$NDS_CA_ID,$NETWORK_NODE1,$NAGRA_CA_ID,$NETWORK_NODE2,$CISCO1_CA_ID,$NETWORK_NODE3,$GOSPELL_CA_ID,$NETWORK_NODE4,$JVM_CA_ID,$NETWORK_NODE5,$NSTV_CA_ID,$NETWORK_NODE6) = split(/,/, $line);

        $PROV_TAG      = trim($PROV_TAG);
        $NDS_CA_ID     = trim($NDS_CA_ID);
        $NETWORK_NODE1 = trim($NETWORK_NODE1);
        $NAGRA_CA_ID   = trim($NAGRA_CA_ID);
        $NETWORK_NODE2 = trim($NETWORK_NODE2);
        $CISCO1_CA_ID  = trim($CISCO1_CA_ID);
        $NETWORK_NODE3 = trim($NETWORK_NODE3);
        $GOSPELL_CA_ID = trim($GOSPELL_CA_ID);
        $NETWORK_NODE4 = trim($NETWORK_NODE4);
        $JVM_CA_ID     = trim($JVM_CA_ID); 
        $NETWORK_NODE5 = trim($NETWORK_NODE5);
        $NSTV_CA_ID     = trim($NSTV_CA_ID);
        $NETWORK_NODE6 = trim($NETWORK_NODE6);
        if (  !(length $PROV_TAG > 0  and 
                length $NDS_CA_ID > 0 and 
                length $NETWORK_NODE1 > 0 and
                length $NAGRA_CA_ID > 0 and 
                length $NETWORK_NODE2 > 0 and
                length $CISCO1_CA_ID > 0 and
                length $NETWORK_NODE3 > 0 and
                length $GOSPELL_CA_ID > 0 and
                length $NETWORK_NODE4 > 0 and
                length $JVM_CA_ID > 0 and 
                length $NETWORK_NODE5 > 0 and
                length $NSTV_CA_ID > 0 and
                length $NETWORK_NODE6 > 0 and) ) 
        {
...
1     MSO_FLD_NETWORK_NODE    STR [0] "$NETWORK_NODE1"
0 MSO_FLD_CATV_PT_DATA  ARRAY [1] allocated 20, used 2
1     MSO_FLD_CA_ID           STR [0] "$NAGRA_CA_ID"
1     MSO_FLD_NETWORK_NODE    STR [0] "$NETWORK_NODE2"
0 MSO_FLD_CATV_PT_DATA  ARRAY [2] allocated 20, used 2
1     MSO_FLD_CA_ID           STR [0] "$CISCO1_CA_ID"
1     MSO_FLD_NETWORK_NODE    STR [0] "$NETWORK_NODE3"
0 MSO_FLD_CATV_PT_DATA  ARRAY [3] allocated 20, used 2
1     MSO_FLD_CA_ID           STR [0] "$GOSPELL_CA_ID"
1     MSO_FLD_NETWORK_NODE    STR [0] "$NETWORK_NODE4"
0 MSO_FLD_CATV_PT_DATA  ARRAY [4] allocated 20, used 2
1     MSO_FLD_CA_ID           STR [0] "$JVM_CA_ID"
1     MSO_FLD_NETWORK_NODE    STR [0] "$NETWORK_NODE5"
0 MSO_FLD_CATV_PT_DATA  ARRAY [5] allocated 20, used 2
1     MSO_FLD_CA_ID           STR [0] "$NSTV_CA_ID"
1     MSO_FLD_NETWORK_NODE    STR [0] "$NETWORK_NODE6"
END
;
