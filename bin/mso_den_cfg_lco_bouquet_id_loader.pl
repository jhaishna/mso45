...
#LCO_ACC_PDP,AREA_CODE,BOUQUET_ID_NDS,BOUQUET_ID_HD_NDS,BOUQUET_ID_NAG,BOUQUET_ID_HD_NAG,BOUQUET_ID_CIS,BOUQUET_ID_HD_CIS,BOUQUET_ID_JVM,BOUQUET_ID_HD_JVM,BOUQUET_ID_GOS,BOUQUET_ID_HD_GOS,POP_ID,BQ_CAT,BQ_SUB_CAT,NW_CAT,NW_SUB_CAT
#33836408345,1,57801,57801,57801,57801,57801,57801,25075,25075,57801,57801,013,2,66,3,6
...
while ($line) 
{
    ...
    else 
    {
        ...
        ($ACC_PDP,$AREA_CODE,$BOUQUET_ID1,$BOUQUET_ID_HD1,$BOUQUET_ID2,$BOUQUET_ID_HD2,$BOUQUET_ID3,$BOUQUET_ID_HD3,$BOUQUET_ID4,$BOUQUET_ID_HD4,$BOUQUET_ID5,$BOUQUET_ID_HD5,$POP_ID, $BQ_CAT,$BQ_SUB_CAT,$NW_CAT,$NW_SUB_CAT) = split(/,/, $line);
		...
		$BOUQUET_ID4 = trim($BOUQUET_ID4);
        $BOUQUET_ID_HD4 = trim($BOUQUET_ID_HD4);
		$BOUQUET_ID5 = trim($BOUQUET_ID5);
        $BOUQUET_ID_HD5 = trim($BOUQUET_ID_HD5);
		$BQ_CAT = trim($BQ_CAT);
		$BQ_CAT = int($BQ_CAT);
        $BQ_SUB_CAT = trim($BQ_SUB_CAT);
    	$BQ_SUB_CAT = int($BQ_SUB_CAT);
		$NW_CAT = trim($NW_CAT);
		$NW_CAT = int($NW_CAT);
        $NW_SUB_CAT = trim($NW_SUB_CAT);
	    $NW_SUB_CAT = int($NW_SUB_CAT);
        $BQ_CAT_HD = $BQ_CAT;
        $BQ_SUB_CAT_HD = $BQ_SUB_CAT;
        $NW_CAT_HD = $NW_CAT;
        $NW_SUB_CAT_HD = $NW_SUB_CAT;
		$POP_ID = trim($POP_ID);
        ...
        if (  !(length $ACC_PDP > 0  and 
                ...
                length $BOUQUET_ID4 > 0 and 
                length $BOUQUET_ID_HD4 > 0 and 
                length $BOUQUET_ID5 > 0 and 
                length $BOUQUET_ID_HD5 > 0 and 
                length $BQ_CAT > 0 and 
                length $BQ_SUB_CAT > 0 and 
                length $NW_CAT > 0 and 
                length $NW_SUB_CAT > 0 and 
                length $BQ_CAT_HD > 0 and
                length $BQ_SUB_CAT_HD > 0 and
                length $NW_CAT_HD > 0 and
                length $NW_SUB_CAT_HD > 0 and
                length $POP_ID > 0 ) ) 
        {
...
0 PIN_FLD_ACCOUNT_OBJ    POID [0] 0.0.0.1 /account $ACC_PDP
...
1     MSO_FLD_NETWORK_NODE    STR [0] "NDS1"
1     MSO_FLD_POPULATION_ID    STR [0] "$POP_ID"
...
1     MSO_FLD_NETWORK_NODE    STR [0] "NAGRA"
1     MSO_FLD_POPULATION_ID    STR [0] "$POP_ID"
1     PIN_FLD_CATEGORY_ID     INT [0] $BQ_CAT
1     PIN_FLD_CATEGORY_VERSION    INT [0] $BQ_SUB_CAT
1     PIN_FLD_REASON_DOMAIN_ID    INT [0] $NW_SUB_CAT
1     PIN_FLD_REASON_ID       INT [0] $NW_CAT
1     PIN_FLD_STRING_ID       INT [0] $BQ_CAT_HD 
1     PIN_FLD_STR_VERSION     INT [0] $BQ_SUB_CAT_HD
1     PIN_FLD_VERSION_ID      INT [0] $NW_CAT_HD
1     PIN_FLD_BASIC_VERSION   INT [0] $NW_SUB_CAT_HD
...
1     PIN_FLD_REASON_DOMAIN_ID    INT [0] 0
1     PIN_FLD_REASON_ID       INT [0] 0
0 PIN_FLD_SERVICE_CODES  ARRAY [3] allocated 20, used 8
1     MSO_FLD_BOUQUET_ID      STR [0] "$BOUQUET_ID4"
1     MSO_FLD_BOUQUET_ID_HD    STR [0] "$BOUQUET_ID_HD4"
1     MSO_FLD_NETWORK_NODE    STR [0] "JVM1"
1     MSO_FLD_POPULATION_ID    STR [0] "$POP_ID"
1     PIN_FLD_CATEGORY_ID     INT [0] 0
1     PIN_FLD_CATEGORY_VERSION    INT [0] 0
1     PIN_FLD_REASON_DOMAIN_ID    INT [0] 0
1     PIN_FLD_REASON_ID       INT [0] 0
0 PIN_FLD_SERVICE_CODES  ARRAY [4] allocated 20, used 8
1     MSO_FLD_BOUQUET_ID      STR [0] "$BOUQUET_ID5"
1     MSO_FLD_BOUQUET_ID_HD    STR [0] "$BOUQUET_ID_HD5"
1     MSO_FLD_NETWORK_NODE    STR [0] "GOSPELL"
1     MSO_FLD_POPULATION_ID    STR [0] "$POP_ID"
1     PIN_FLD_CATEGORY_ID     INT [0] 0
1     PIN_FLD_CATEGORY_VERSION    INT [0] 0
1     PIN_FLD_REASON_DOMAIN_ID    INT [0] 0
