...
while ($line) 
{
    ...
    else 
    {
        ...
        ($ACC_PDP,$BOUQUET_ID1,$BOUQUET_ID2,$BOUQUET_ID3,$BOUQUET_ID4,,$BOUQUET_ID_HD1,$BOUQUET_ID_HD2,$BOUQUET_ID_HD3,$BOUQUET_ID_HD4,$NDS2_POP_ID,$GOSPELL2_POP_ID,$JVM2_POP_ID,$_NAGRA2_POP_ID,$BQ_CAT,$BQ_SUB_CAT,$NW_CAT,$NW_SUB_CAT,$AREA_CODE) = split(/,/, $line);
        ...
        $_NAGRA2_POP_ID = trim($_NAGRA2_POP_ID);
        $BQ_CAT = trim($BQ_CAT);
        $BQ_CAT = int($BQ_CAT);
        ...
        if (  !(length $ACC_PDP > 0  and 
                ...
                length $JVM2_POP_ID > 0  and
                length $_NAGRA2_POP_ID > 0 and
                length $BQ_CAT > 0 and
...
1     MSO_FLD_NETWORK_NODE    STR [0] "NAGRA2"
1     MSO_FLD_POPULATION_ID    STR [0] "$_NAGRA2_POP_ID"
