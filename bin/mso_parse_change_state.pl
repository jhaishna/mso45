...
#        print "\nInvalid input parameter";
        print "\nUsage: mso_parse_update_state.pl -device_type <device_type> -input <input file name> ";
        print "\n       Ex: mso_parse_update_state.pl -device_type /device/stb -input stb_input_file \n";
        exit 1;
}
...
else
{
     ...
     #$app_type  = 'mso_device_change_state';
}
...
while (<CONF>) {
    $conf = $_ ;
    if ($conf  =~ /mso_parse_change_state/ ) {
        if ($conf  =~ /loglevel/ ) {
...
if (length $LOGLEVEL <= 0) {
    print "\nSEVERE: loglevel not configured for mso_parse_update_state in pin.conf file.. Exiting\n";
    exit 1;
}
...
$input_file_name = $ARGV[3];
$template_file = "./template/" . $app_type . "/change_state_" . $app_type . ".TMPL";
...
1   PIN_FLD_POID  POID [0] 0.0.0.1 $device_type -1
0 PIN_FLD_RESULTS       ARRAY [0]
1  PIN_FLD_POID        POID [0] NULL
END
;
        ...
        my $location_update_flist = << "END"
0 PIN_FLD_POID            POID [0] @Device_out[3]
0 PIN_FLD_NEW_STATE        INT [0] $state_id
END
;
        # Convert the flist string into an actual FList
        $location_update_flistp = pcmif::pin_perl_str_to_flist($location_update_flist, $db_no, $ebufp);
        if (pcmif::pcm_perl_is_err($ebufp)) 
        {
        ...
        $location_update_out_flistp = pcmif::pcm_perl_op($pcm_ctxp,2701,
                                                 0, $location_update_flistp, $ebufp);
        ...
        if (pcmif::pcm_perl_is_err($ebufp)) 
        {
            ...
            # Break and process next line
            #==============================================
            pcmif::pin_flist_destroy($location_update_flistp);
            pcmif::pcm_perl_destroy_ebuf($ebufp);
            $ebufp = pcmif::pcm_perl_new_ebuf();
            goto NEXT_RECORD;
        }

        $location_update_out_flist = pcmif::pin_perl_flist_to_str($location_update_out_flistp, $ebufp);

        &log ($DEBUG, "Object created successfully :" . "\n$location_update_out_flist" );
        $success_count = $success_count + 1;
        pcmif::pin_flist_destroy($location_update_flistp);
        pcmif::pin_flist_destroy($location_update_out_flistp);
