...
#        print "\nInvalid input parameter";
        print "\nUsage: mso_parse_update_location.pl -device_type <device_type> -input <input file name> ";
        print "\n       Ex: mso_parse_update_location.pl -device_type /device/stb -input stb_input_file \n";
        exit 1;
}
...
while (<CONF>) {
    $conf = $_ ;
    if ($conf  =~ /mso_parse_update_location/ ) {
        if ($conf  =~ /loglevel/ ) {
...
if (length $LOGLEVEL <= 0) {
    print "\nSEVERE: loglevel not configured for mso_parse_update_location in pin.conf file.. Exiting\n";
    exit 1;
}
...
0 PIN_FLD_ACCOUNT_OBJ     POID [0] @LCO_out[3] 
0 PIN_FLD_SOURCE           STR [0] "$account_no"
END
;
        # Convert the flist string into an actual FList
        ...
        $location_update_out_flistp = pcmif::pcm_perl_op($pcm_ctxp, 5,
