...
if [ $Rstatus -eq 0 ] ; then
   echo "`date`:Testnap Connection Success for bulk_refund_load in /home/pin/opt/portal/7.5/mso/bin/"
else
   echo "`date`:Testnap Connection Failed so job skipped for bulk_refund_load in /home/pin/opt/portal/7.5/mso/bin/" >> /data/opt/portal/7.5/var/cron_global/global_cron_tab.log
   exit;
fi
...
if [ $Rstatus -eq 0 ] ; then
   ...
   echo "`date`:Testnap Connection Failed so job skipped for bulk_refund_load in PIN_HOME/mso/apps/mso_bulk_AR" >> /data/opt/portal/7.5/var/cron_global/global_cron_tab.log
   exit;
fi
...
do
        ...
        then
                if [[ $result -eq $fail ]];
                then
                        echo "`date`:bulk_refund_load Job Failed" >> /data/opt/portal/7.5/var/cron_global/global_cron_tab.log
                else
                        echo "`date`:bulk_refund_load Job Success" >> /data/opt/portal/7.5/var/cron_global/global_cron_tab.log
                fi
        fi
