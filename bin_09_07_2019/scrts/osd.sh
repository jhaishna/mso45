...
do
        ...
        then
                if [[ $result -eq $fail ]];
                then
                        echo "`date`:bulk_osd Job for PK/CISCO/CISCO1 Failed" >> /data/opt/portal/7.5/var/cron_global/global_cron_tab.log
                else
                        echo "`date`:bulk_osd Job for PK/CISCO/CISCO1 Success" >> /data/opt/portal/7.5/var/cron_global/global_cron_tab.log
                fi
        fi
