...
prog_name='SWAP'
logfile='/home/pin/opt/portal/7.5/mso/apps/mso_bulk_audit/swap.log'
...
do
        ...
        then
                if [[ $result -eq $fail ]];
                then
                        echo "`date`:bulk_swap_cpe Job Failed" >> /data/opt/portal/7.5/var/cron_global/global_cron_tab.log
                else
                        echo "`date`:bulk_swap_cpe Job Success" >> /data/opt/portal/7.5/var/cron_global/global_cron_tab.log
                fi
        fi
