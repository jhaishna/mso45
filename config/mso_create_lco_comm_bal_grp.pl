...
#==================================================================================
# Load the pakcage mapipng file create LCO Commission /balance_group object for LCO 
#==================================================================================
use strict;
use warnings;
...
foreach $plan_id (@lines)	
{
	if ($plan_id !~ /^\#/){
		...
		$check = input_valid($billinfo_obj, 2);
		if ($check == 0){
			log_msg(3, "Invalid Billinfo given");
			print ERROR "$plan_id\n";
			$error = $error+1;
			next;
		}	
		...
		my $bgrp_name = "LCO Commission";
		my $res;
		$res = create_balgrp($acct_poid, $billinfo_obj, $bgrp_name);
		$bgrp_name = "LCO Balance";
		$res = create_balgrp($acct_poid, $billinfo_obj, $bgrp_name);
		if ($res == 1){
			$success = $success+1;
		}
...
print "Total number of records already exists and no need update: $noupdate\n" ;
...
{
     my $acct_poid = shift;
     my $billinfo_obj = shift;
     my $bgrp_name = shift;
     my $result = 0;

     #print "ACCOUNT POID: $acct_poid, Billinfo: $billinfo_obj \n";
...
1         PIN_FLD_NAME	STR [0] "$bgrp_name"
0 PIN_FLD_RESULTS      ARRAY [0]
1     PIN_FLD_POID          POID [0] 0.0.0.0  0 0
XXX
;
		log_msg(1,"Input flist for $bgrp_name balance_group search: \n $f1");
		...
		log_msg(1,"$bgrp_name Balance group search output: \n $out");

		pcmif::pin_flist_destroy($flistp);
		pcmif::pin_flist_destroy($out_flistp);

		if ($out =~ "/balance_group") {
			log_msg(1,"$bgrp_name balance_group already exists ");
			$result = 2;
...
0 PIN_FLD_NAME 		STR [0] "$bgrp_name"
XXX
;
			...
			$out = pcmif::pin_perl_flist_to_str($out_flistp, $ebufp);
			log_msg(1,"creating new $bgrp_name balance_group output flist: \n $out");

			pcmif::pin_flist_destroy($flistp);
			pcmif::pin_flist_destroy($out_flistp);
		
			if ($out =~ "/balance_group") {
				print "$bgrp_name balance_group $acct_poid\n" ;
				log_msg(1,"$bgrp_name balance_group is created for : $acct_poid ");
				$result = 1;
			}
